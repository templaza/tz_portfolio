...
require_once JPATH_ADMINISTRATOR.'/components/com_tz_portfolio_plus/models/article.php';
...
class TZ_Portfolio_PlusModelForm extends TZ_Portfolio_PlusModelArticle
{
    ...
    function getAttachment(){
        ...
        if($id){
            $query  = 'SELECT attachfiles,attachtitle,attachold FROM #__tz_portfolio_plus_xref_content'
                .' WHERE contentid = '.$id;
            $db     = JFactory::getDbo();
            $db -> setQuery($query);
            if(!$db -> query()){
    ...
    public function getTags(){
        ...
        if($artid){
            $query  = 'SELECT t.title FROM #__tz_portfolio_plus_tags AS t'
                      .' LEFT JOIN #__tz_portfolio_plus_tags_xref AS x ON x.tagsid=t.id'
