...
class JFormFieldModal_Category extends JFormField
{
	...
	{
		$extension 		= $this->element['extension'] ? (string) $this->element['extension'] : (string) JFactory::getApplication()->input->get('extension', 'com_tz_portfolio_plus');
		...
		$html = array();
		$link = 'index.php?option=com_tz_portfolio_plus&amp;view=categories&amp;layout=modal&amp;tmpl=component&amp;extension=' . $extension . '&amp;function=jSelectCategory_' . $this->id;
		...
		if ($allowEdit)
		{
			$html[] = '<a class="btn hasTooltip' . ($value ? '' : ' hidden') . '" href="index.php?option=com_tz_portfolio_plus&layout=modal&tmpl=component&task=category.edit&id=' . $value . '" target="_blank" title="' . JHtml::tooltipText('COM_CATEGORIES_EDIT_CATEGORY') . '" ><span class="icon-edit"></span> ' . JText::_('JACTION_EDIT') . '</a>';
		}
