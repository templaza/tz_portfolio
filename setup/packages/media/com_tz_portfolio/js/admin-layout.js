...
(function($){
    ...
    var $tpLayoutAdmin = $.tpLayoutAdmin  = function(options,el) {

        var $this   = $(el),
            $var    = $.extend(true,$tpLayoutAdmin.defaults,options);
        ...
        $.tpLayoutAdmin.tzTemplateSubmit = function () {
        ...
        var popover = function () {
            ...
            $('a[rel="popover"]').popover({
                html: true,
                sanitize: false,
                template: '<div class="popover" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-header"></h3><div class="popover-body"></div><div class="px-3 pb-3"><a class="btn btn-primary sp-popover-apply" onclick="jQuery(this).closest(\'.popover\').prevAll(\'a[rel=popover]\').popover(\'hide\');  jQuery(this).closest(\'.popover\').prevAll(\'a[rel=popover]\').show();"><i class="icon-ok"></i> Apply</a> <a class="btn btn-danger sp-popover-close" onclick="jQuery(this).closest(\'.popover\').prevAll(\'a[rel=popover]\').popover(\'hide\'); jQuery(this).closest(\'.popover\').prevAll(\'a[rel=popover]\').show();"><i class="icon-remove"></i> Close</a></div></div>',
                content: function (event) {

                    var __el_pop = $(this);

                    if((typeof this == "undefined" || this === window) && typeof event !== "undefined"){
                        __el_pop    = $(event);
                    }
                    ...
                    __el_pop.closest(".column").delegate("input:checkbox", 'click', function (event) {
                    ...
                    $(id).find("joomla-tab").children(":not(joomla-tab-element)").remove();
                    return $(id).html();
                }
            }).on("click",function () {
                $(this).show();
            ...
            $('a[rel="rowpopover"]').popover({
                ...
                content: function (event) {
                    ...
                    setTimeout(function ($this, value) {
                        ...
                        $this.parent().find('>.popover .rowbackgroundcolor').minicolors({
                            format: "rgb",
                            opacity: true,
                            change: function(hex, opacity) {
                                $(this).closest('.popover').parent().prev()
                                    .find('>span.rowdocs>.rowbackgroundcolorinput').val(hex);
                            }
                        });
                    ...
                    setTimeout(function ($this, value) {
                        ...
                        $this.parent().find('>.popover .rowtextcolor').minicolors({
                            format: "rgb",
                            opacity: true,
                            change: function(hex, opacity) {
                                $(this).closest('.popover').parent().prev()
                                    .find('>span.rowdocs>.rowtextcolorinput').val(hex);
                            }
                        });
                    ...
                    setTimeout(function ($this, value) {
                        ...
                        $this.parent().find('>.popover .rowlinkcolor').minicolors({
                            format: "rgb",
                            opacity: true,
                            change: function(hex, opacity) {
                                $(this).closest('.popover').parent().prev()
                                    .find('>span.rowdocs>.rowlinkcolorinput').val(hex);
                            }
                        });
                    ...
                    setTimeout(function ($this, value) {
                        ...
                        $this.parent().find('>.popover .rowlinkhovercolor').minicolors({
                            format: "rgb",
                            opacity: true,
                            change: function(hex, opacity) {
                                $(this).closest('.popover').parent().prev()
                                    .find('>span.rowdocs>.rowlinkhovercolorinput').val(hex);
                            }
                        });
                    ...
                    __el_pop.parent().delegate("input:checkbox", 'click', function (event) {
        ...
        $("#content,#element-box").delegate("a.add-row", 'click', function () {
            var $item = $(this);
            $.get('index.php?option=com_tz_portfolio&view=layout&layout=new-row&format=tpl&time=' + $.now(), function ($row) {
        ...
        var addColumn = function ($selector) {
            $.get("index.php?option=com_tz_portfolio&view=layout&layout=new-column&format=tpl", function ($column) {
        ...
        var addRowInColumn = function () {
            $("#content,#element-box").delegate("a.add-rowin-column", 'click', function () {
                var $this = $(this);
                $.get("index.php?option=com_tz_portfolio&view=layout&layout=new-row&format=tpl"
    ...
    $.fn.tpLayoutAdmin    = function(options){
        if (options === undefined) options = {};
        if (typeof options === "object") {
            return new $.tpLayoutAdmin(options,this);
        }
    };
