...
//
//$wa -> addInlineScript('(function($, TZ_Portfolio){
//        "use strict";
//        TZ_Portfolio.dialogAjax(["'.$this -> getName().'.approve", "'.$this -> getName().'.reject"]);
//    })(jQuery, window.TZ_Portfolio);', ['com_tz_portfolio.core']);
                            ...
                            <tr class="row<?php echo $i % 2; ?>" data-draggable-group="<?php echo $item->catid; ?>">
                                ...
                                <td class="has-context">
                                    ...
                                    if(($canApprove && ($canEdit || $canEditOwn || $item -> state == 3 || $item -> state == 4)) ||
                                        ...
                                        ?>
                                        <a href="<?php echo Route::_('index.php?option=com_tz_portfolio&task=article.edit&id='.$item->id);?>">
                                            <?php echo $this->escape($item->title); ?></a>
                                    <?php }else{ ?>
                                        <?php echo $this->escape($item->title); ?>
                                    <?php } ?>
                                    <?php if(isset($item -> rejected_id) && $item -> rejected_id && in_array($item -> state, array(-3,3,4))){ ?>
                                        <span class="badge text-bg-danger"><?php echo Text::_('COM_TZ_PORTFOLIO_REJECTED'); ?></span>
                                    <?php } ?>
                                    <?php
                                    if($filterPublished === '*'){?>
                                        <?php if($item -> state == -3){ ?>
                                            <span class="badge text-bg-secondary"><?php echo Text::_('COM_TZ_PORTFOLIO_DRAFT'); ?></span>
                                        <?php } ?>
                                        <?php if($item -> state == 3){ ?>
                                            <span class="badge text-bg-warning"><?php echo Text::_('COM_TZ_PORTFOLIO_PENDING'); ?>...</span>
                                        <?php } ?>
                                        <?php if($item -> state == 4){ ?>
                                            <span class="badge text-bg-info"><?php echo Text::_('COM_TZ_PORTFOLIO_UNDER_REVIEW'); ?></span>
                                        <?php } ?>
                                    <?php } ?>
                                    <div class="small">
                                        <div class="clearfix">
