...
<div class="">
    <button type="button" data-toggle="collapse" data-target="#collapseUpload" data-bs-toggle="collapse" data-bs-target="#collapseUpload" class="btn btn-success">
        <span class="fas fa-upload"></span> <?php echo Text::_('COM_TZ_PORTFOLIO_UPLOAD_PRESET');?></button>
    <div id="collapseUpload" class="collapse">

        <div class="control-group mt-px-18">
            <label for="upload-preset-file" class="list-inline"><?php echo Text::_('COM_TZ_PORTFOLIO_UPLOAD_FILE'); ?></label>
            <input type="file" id="upload-preset-file" name="Filepreset[]" multiple />
            <button type="button" class="btn btn-primary" id="upload-preset-submit"><i class="tpr fa-caret-square-right"></i>  <?php
                echo Text::_('COM_TZ_PORTFOLIO_START_UPLOAD'); ?></button>
            <p class="help-block">
                <?php $maxSize = Utility::getMaxUploadSize(); ?>
...
<div class="presets">
    <div class="alert alert-warning">
        <?php echo Text::_('COM_TZ_PORTFOLIO_LOAD_PRESET_DESCRIPTION');?>
    </div>
    <?php foreach($presets as $preset){?>
        <div class="preset<?php echo (isset($this -> item -> preset)
            ...
            <div class="preset-screenshot<?php echo (!isset($preset -> image) || (isset($preset -> image) && !$preset -> image))?' background':''; ?>">
                ...
                <img src="<?php echo TZ_PortfolioUri::root().'/'.$preset -> image;?>" alt="<?php echo $preset -> name;?>">
                <?php }else{?>
                    ...
                    <div class="preset-bar-table">
                        <div class="preset-bar-table-cell">
                            <span data-preset="<?php echo $preset -> name;?>" data-target="#loadPreset" data-toggle="modal" data-bs-toggle="modal"
                                  class="load-preset btn btn-warning btn-small btn-sm"><?php echo Text::_('COM_TZ_PORTFOLIO_LOAD_PRESET');?></span>
                            <?php if(isset($preset -> doc_link) && $preset -> doc_link){ ?>
                                <a target="_blank" class="btn btn-primary btn-small btn-sm"
                                   href="<?php echo $preset -> doc_link;?>"><?php echo Text::_('JTOOLBAR_HELP');?></a>
                            <?php }
                            if(isset($preset -> demo_link) && $preset -> demo_link){
                                ?>
                                <a target="_blank" class="btn btn-success btn-small btn-sm"
                                   href="<?php echo $preset -> demo_link;?>"><?php echo Text::_('COM_TZ_PORTFOLIO_LIVE_PREVIEW');?></a>
                            <?php }?>
                        </div>
                    </div>
                </div>
            ...
            <div class="preset-bottom">
                ...
                <div class="action">
                    <span data-preset="<?php echo $preset -> name;?>"
                       title="<?php echo Text::_('COM_TZ_PORTFOLIO_DOWNLOAD_PRESET_DESCRIPTION');?>"
                       class="fas fa-download downloadpreset hasTooltip"></span>
                    <i data-preset="<?php echo $preset -> name;?>" data-target="#removePreset" data-toggle="modal"
                       ...
                       title="<?php echo Text::_('COM_TZ_PORTFOLIO_REMOVE_PRESET_DESCRIPTION');?>"
                       class="fas fa-times removepreset hasTooltip"></i>
                </div>
            </div>
        </div>
    <?php }?>
