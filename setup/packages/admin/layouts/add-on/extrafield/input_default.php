    ...
    if(!$head) {
        ...
        <tr>
            ...
            <td>
                ...
                echo $id_head_text; ?>"><?php echo Text::_('COM_TZ_PORTFOLIO_OPTION_FIELD_VALUE_LABEL'); ?></label>
            </td>
            <td>
            ...
            <td>
                <button type="button" class="btn btn-danger btn-mini btn-sm tz_remove-option"><i class="icon-minus"></i><?php
                    echo Text::_('COM_TZ_PORTFOLIO_REMOVE'); ?></button>
            </td>
        </tr>
        <?php
        ...
        $wa -> addInlineStyle('#jform_' . $group . ' .table{
                        margin-top: 5px;
                    }');
        $wa -> addInlineScript('
        (function($){
            $(document).ready(function(){
                function tz_extrafields(){
                    ...
                    $("#jform_' . $group . ' .tz_add-option").on("click",function(e){
                        var html    = "' . TZ_PortfolioHelper::jsAddSlashes($htmlHead) . '";
...
<button type="button" class="btn btn-secondary btn-mini tz_add-option"><i class="icon-plus"></i><?php
    echo Text::_('COM_TZ_PORTFOLIO_ADD_AN_OPTION'); ?></button>
<div class="max-height-300">
    <table class="table table-striped table-bordered">
        <thead>
        <tr>
            <th><?php echo Text::_('COM_TZ_PORTFOLIO_SORT'); ?></th>
            <th><?php echo Text::_('COM_TZ_PORTFOLIO_FIELD_TEXT'); ?>
                <span class="star">&nbsp;*</span>
            </th>
            <th><?php echo Text::_('COM_TZ_PORTFOLIO_VALUE');?></th>
            <th><?php echo Text::_('JDEFAULT'); ?></th>
        ...
        <?php if($fieldValues && !is_string($fieldValues)){?>
            <?php foreach($fieldValues as $key => $value){
                $name_text  = $formControl.'['
                    .$group.']['.$key.'][text]';
                $id_text    = ApplicationHelper::stringURLSafe($name_text);
                $id_text    = preg_replace('#\W#', '_', $id_text);

                $name_value = $formControl.'['
                    .$group.']['.$key.'][value]';
                $id_value   = ApplicationHelper::stringURLSafe($name_value);
            ...
            ?>
                <tr>
                    ...
                    <td>
                        ...
                        ?>"><?php echo Text::_('COM_TZ_PORTFOLIO_OPTION_FIELD_VALUE_LABEL'); ?></label>
                    </td>
                    <td>
