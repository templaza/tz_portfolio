...
class LayoutModel extends AdminModel
{
    ...
    protected function populateState(){
        ...
        $this -> setState('layout.style',null);
        $this -> setState('template.rowincolumn',$input -> get('rowincolumn', false, 'bool'));
    }

    public function getTable($name = 'Style', $prefix = 'Administrator', $options = array())
    {
    ...
    public function getForm($data = array(), $loadData = true){
        ...
        /* @var Form $form */
        // Replace COM_TZ_PORTFOLIO_PLUS to COM_TZ_PORTFOLIO
        $formXML    = $form -> getXml();
        $xml        = $formXML -> asXML();

        $xml    = str_replace(array('COM_TZ_PORTFOLIO_PLUS', 'com_tz_portfolio_plus'),
            array('COM_TZ_PORTFOLIO', 'com_tz_portfolio'), $xml);

        $org_data   = $form -> getData();

        /* Reset form to accept string replace*/
        $form -> reset(true);
        $form -> load($xml, true);
        $form -> bind($org_data);
    ...
    {
        $template       = $this->getState('layout.style');
        $lang           = Factory::getApplication() -> getLanguage();

        $template_path  = COM_TZ_PORTFOLIO_PATH_SITE.DIRECTORY_SEPARATOR.'styles'
            .DIRECTORY_SEPARATOR.$template;

        $formFile = Path::clean($template_path.DIRECTORY_SEPARATOR.'template.xml');

        // Load the core and/or local language file(s).
        TZ_PortfolioTemplate::loadLanguage($template);
...
//                        StylesHelper::syncColumnLayoutToUIkit($item->layout);
                    }
                }
    ...
    public function save($data)
    {
        ...
        if(isset($post['jform']['attrib']) && $attrib = $post['jform']['attrib']){
            ...
            $pathfile   = COM_TZ_PORTFOLIO_MEDIA_PATH.'/style/default.json';
                ...
                if(is_array($presets) && count($presets)){
                    ...
                    if(count($presets) && isset($presets['name'])){
                        ...
                        if($name && !empty($name)) {
                            ...
                            $preset_name            = ApplicationHelper::stringURLSafe($name);
                            ...
                            $path   = COM_TZ_PORTFOLIO_STYLE_PATH
            ...
            if(file_exists($pathfile)){
                ...
                if(is_object($configLayout) && isset($configLayout -> layout) && $configLayout -> layout){
                    if(is_string($configLayout -> layout)){
                        $clayout    = json_decode($configLayout -> layout);
//                        StylesHelper::syncColumnLayoutToUIkit($clayout);

                        return $clayout;
                    }

//                    StylesHelper::syncColumnLayoutToUIkit($configLayout -> layout);
//                    return $configLayout -> layout;
                    return (array) $configLayout -> layout;
                }
//                StylesHelper::syncColumnLayoutToUIkit($configLayout);
                return $configLayout;
            }
        }
        return $layout;
...
//                    return false;
                ...
                if ($table->home != '0')
                {
//                    JError::raiseWarning(SOME_ERROR_NUMBER, Jtext::_('COM_TZ_PORTFOLIO_STYLE_STYLE_CANNOT_DELETE_DEFAULT_STYLE'));

                    throw new \Exception(Text::_('COM_TZ_PORTFOLIO_STYLE_STYLE_CANNOT_DELETE_DEFAULT_STYLE'));
//                    return false;
                }
