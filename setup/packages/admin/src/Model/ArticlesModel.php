    ...
    protected function getListQuery()
    {
        ...
        if (Associations::isEnabled())
        {
            $query->select('COUNT(asso2.id)>1 as association')
                ->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_tz_portfolio.article.item'))
        ...
        if (is_numeric($categoryId)) {
            $cat_tbl = Table::getInstance('Category', 'Administrator');
    ...
    public function getItems()
    {
        ...
        if($items){
            ...
            foreach($items as &$item){
                $categories			= TZ_PortfolioHelperCategories::getCategoriesByArticleId($item -> id, 0);
                ...
                if(isset($item -> type) && in_array($item -> type, $values)){
                    ...
                    $item -> type  = Text::_('COM_TZ_PORTFOLIO_OPTION_NONE_MEDIA');
                }
            }
        }
        return $items;
