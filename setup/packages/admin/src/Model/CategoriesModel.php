...
class CategoriesModel extends ListModel
{
    ...
    protected function populateState($ordering = 'a.lft', $direction = 'asc')
    {
        ...
        $extension = $app->getUserStateFromRequest('com_tz_portfolio.categories.filter.extension',
            'extension', 'com_tz_portfolio', 'cmd');
...
//        $catid  = $this -> getState('filter.group');
        ...
        if($rows2 = $db -> loadObjectList()){
            ...
            $groups[]  = Text::_('COM_TZ_PORTFOLIO_OPTION_INHERIT_FROM_CATEGORY');
...
//        $query -> join('LEFT','#__tz_portfolio_plus_categories AS c ON c.catid = a.id');
        ...
        if ($extension = $this->getState('filter.extension')) {
            $query->where('(a.extension = '.$db->quote($extension).' OR a.extension ='
            ...
            foreach($items as &$item){
                ...
                if($catid = (int) $registry -> get('inheritFrom')){
                    ...
                    $query	= $db -> getQuery(true)
                        ...
                        -> where('extension='.$db -> quote('com_tz_portfolio'))
                        -> where('id = '.$catid);
                    $db -> setQuery($query);
