...
class CategoryeditField extends ListField
{
	...
	protected function getOptions()
	{
		...
		if ($this->element['parent'] || ($jinput->get('option') == 'com_tz_portfolio'
				&& $jinput -> get('view') == 'categories'))
		{
			$oldCat = $jinput->get('id', 0);
			$oldParent = $this->form->getValue($name, 0);
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('extension', 'com_tz_portfolio');
		}
		else
		...
		{
			$subQuery->where('(extension = ' . $db->quote($extension) . ' OR extension ='.
                $db -> quote('com_tz_portfolio_plus').' OR parent_id = 0)');
		}
		else
		{
			$subQuery->where('(extension = ' . $db->quote($extension) . ' OR extension = '.
