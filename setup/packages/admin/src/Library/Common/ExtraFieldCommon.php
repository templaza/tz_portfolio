...
class ExtraFieldCommon implements BootableExtensionInterface,
...
//        if (!is_object($field) || ($field instanceof \Joomla\Event\Dispatcher))
//        {
//            return false;
//        }
...
//            $this -> field  = $field;
    ...
    public function loadLanguage($fieldFolder){
        ...
        {
            $fieldXmlPath = COM_TZ_PORTFOLIO_ADDON_PATH.DIRECTORY_SEPARATOR.'extrafields'
                .DIRECTORY_SEPARATOR . $fieldFolder . DIRECTORY_SEPARATOR . $fieldFolder . '.xml';

            if (file_exists($fieldXmlPath))
            {
                $lang           = Factory::getApplication() -> getLanguage();
                $tag            = $lang -> getTag();

                $langPath   = COM_TZ_PORTFOLIO_ADDON_PATH.DIRECTORY_SEPARATOR.'extrafields'
                    .DIRECTORY_SEPARATOR . $fieldFolder;
                $prefix = 'tp_addon_extrafields_';

                if(!file_exists($langPath.'/language/'.$tag.'/'.$tag.'.'.$prefix.$fieldFolder.'.ini')){
    ...
    public function getInputDefault($group = null){
        ...
        $input_def_path = COM_TZ_PORTFOLIO_ADDON_PATH.DIRECTORY_SEPARATOR.'extrafields'
            .DIRECTORY_SEPARATOR.$this -> fieldname.DIRECTORY_SEPARATOR.'tmpl'.DIRECTORY_SEPARATOR.'input_default.php';

        if(file_exists($input_def_path)){
            ob_start();
            require_once $input_def_path;
            $html   = ob_get_contents();
            ob_end_clean();
        }else{
            $html   = LayoutHelper::render('add-on.extrafield.input_default',
                array(
                    'head'              => $this -> head,
                    'group'             => $group,
    ...
    public function getLabel(){
        ...
        if($this -> params -> get('show_label',1)) {
            ...
            if ($desc = $this->description) {
                ...
                $text   = htmlspecialchars(Text::_($this->getTitle()));
                $title  = ' title="' . HTMLHelper::tooltipText(trim($text, ':'), $desc, 0, 0) . '"';
            }
    ...
    public function getInput($fieldValue = null, $group = null)
    {
        ...
        $this -> setAttribute('class', 'form-control uk-input', 'input');
            ...
            if (count($_options)) {
                ...
                foreach ($_options as $option) {
                    ...
                    if($this -> isSearchLink()){
                        $text   = '<a href="'.Route::_(RouteHelper::getSearchRoute().'&fields['.
    ...
    protected function getTmplFile($file = 'output', $class = null)
    {
        ...
        $client = Factory::getApplication() -> isClient('administrator')?'admin':'';

        if (!is_null($class))
        {
            $reflect = new \ReflectionClass($class);
            $folder = strtolower($reflect -> getShortName());
        ...
        if($folder) {
            ...
            $tmpPath    = '';
            if($client){
                $tmpPath    = Path::clean(COM_TZ_PORTFOLIO_ADDON_PATH . '/' . $this -> group
                    . '/' .$folder.'/'.$client.'/tmpl/'.$file);
            }

            if(!file_exists($tmpPath)){
                $tmpPath = Path::clean(COM_TZ_PORTFOLIO_ADDON_PATH . '/' . $this->group
                    . '/' . $folder . '/tmpl/core/' . $file);
            }
            ...
            if(isset($template -> home_path) && $template -> home_path){
                $_tmpPath    = $template -> home_path. '/'
                    .$layoutPrefix.$this -> group.'_'. $this -> type. '/' . $file;
                if(!file_exists($_tmpPath)) {
                    $_tmpPath = $template->home_path . '/'
                        .$orgLayoutPrefix. $this->group . '_' . $this->type . '/' . $file;
                }
                if(file_exists($_tmpPath)){
                    $tmpPath    = $_tmpPath;
                }
            ...
            if ($tplparams->get('override_html_template_site', 0)) {
                ...
                $_tmpPath    = JPATH_SITE . '/templates/' . $_template . '/html/com_tz_portfolio/'
                    .$layoutPrefix.$this -> group.'_' . $this -> type. '/'.$file;
                if(!file_exists($_tmpPath)) {
                    $_tmpPath = JPATH_SITE . '/templates/' . $_template . '/html/com_tz_portfolio/'
                        .$orgLayoutPrefix. $this->group . '_' . $this->type . '/' . $file;
    ...
    public function loadArticle($article, $resetCache = false)
    {
        ...
        {
            $_article = ArticleHelper::getArticleById($article, $resetCache);
    ...
    public function setAttribute($name, $value, $type = 'output'){
        ...
        if (is_null($value))
        {
            ...
            $this->attributes[$type] = new Registry($attributeArray);
    ...
    public function isPublished()
    {
        ...
        if (!isset(self::$cache[$storeId]))
        {
            ...
            if($this -> groupid){
                if($fieldGroupObj = ExtraFieldsFrontHelper::getFieldGroupsById($this->groupid)){
                    self::$cache[$storeId] = false;
    ...
    public function onSaveArticleFieldValue($value)
    {
        ...
        if($countData > 0){
            ...
            if($groupid    = ExtraFieldsFrontHelper::getFieldGroupsByArticleId($this -> article_id)) {
    ...
    public function prepareForm(&$form, $data){
        $name       = $form -> getName();
        $path       = Path::clean(COM_TZ_PORTFOLIO_ADDON_PATH.'/'.$this -> group.'/'.$this -> type );
        $viewName   = str_replace('com_tz_portfolio.', '', $name);

        Form::addFormPath($path . '/admin/models/forms');
        Form::addFormPath($path . '/admin/model/form');
