...
class AddonAdapter extends PluginAdapter
{
...
//        $this->type = 'tz_portfolio_plus-plugin';
    ...
    protected function setupInstallPaths()
    {
        ...
        $this->parent->setPath('extension_root', COM_TZ_PORTFOLIO_ADDON_PATH . '/' . $this->group . '/' . $this->element);
    }
    ...
    public function getElement($element = null)
    {
        if (!$element) {
            ...
            if (count($this->getManifest()->files->children())) {
                ...
                $type = str_replace('tz_portfolio-', '', $type);
                ...
                foreach ($this->getManifest()->files->children() as $file) {
                    ...
                    if (empty($element) && (string)$file->attributes()->plugin) {
    ...
    protected function storeExtension($deleteExisting = false)
    {
        ...
        if ($deleteExisting) {
            ...
            $query = $db->getQuery(true)
                ...
                ->from($db->quoteName('#__tz_portfolio_plus_plus_extensions'))
            ...
            if (!empty($extension_ids)) {
                foreach ($extension_ids as $eid) {
                    ...
                    $extensionTable = Table::getInstance('Extensions', 'TemPlaza\Component\TZ_Portfolio\Administrator\Table\\');
...
//        $row = Table::getInstance('Extensions', 'TemPlaza\Component\TZ_Portfolio\Administrator\Table\\');
        $mvc = Factory::getApplication() -> bootComponent('com_tz_portfolio') -> getMVCFactory();
        $row    = $mvc -> createTable('Extensions', 'Administrator');

        if (!$row->load((int)$id)) {
            Log::add(Text::_('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_ERRORUNKOWNEXTENSION'), Log::WARNING, 'jerror');
        ...
        if ($row->protected) {
            Log::add(Text::sprintf('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_WARNCOREPLUGIN', $row->name),
                Log::WARNING, 'jerror');
        ...
        if (trim($row->folder) == '') {
            Log::add(Text::_('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_FOLDER_FIELD_EMPTY'), Log::WARNING, 'jerror');

            return false;
        }

        // Set the plugin root path
        $this->parent->setPath('extension_root', COM_TZ_PORTFOLIO_ADDON_PATH
        ...
        $this->parent->setPath('source', COM_TZ_PORTFOLIO_ADDON_PATH . '/' . $row->folder . '/' . $row->element);
        $this->loadLanguage(COM_TZ_PORTFOLIO_ADDON_PATH . '/' . $row->folder . '/' . $row->element);
        ...
        if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight')) {
            if ($this->parent->manifestClass->preflight($this->route, $this) === false) {
                ...
                $this->parent->abort(Text::_('JLIB_INSTALLER_ABORT_PLG_INSTALL_CUSTOM_INSTALL_FAILURE'));
        ...
        if ($utfresult === false) {
            ...
            $this->parent->abort(Text::sprintf('JLIB_INSTALLER_ABORT_PLG_UNINSTALL_SQL_ERROR', $db->stderr(true)));
        ...
        if($row -> type != 'tz_portfolio_plus-plugin' || ($row -> type == 'tz_portfolio_plus-plugin'
                && !file_exists(JPATH_ADMINSTRATOR
