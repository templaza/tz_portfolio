...
class TPFormController extends FormController {
    /**
    ...
    public function add()
    {
        ...
        $this->setRedirect(Route::_($this -> getAddonRedirect(). $this->getRedirectToItemAppend()
            , false));
    ...
    protected function allowAdd($data = array())
    {
        ...
        $asset      = Table::getInstance('Asset','Joomla\CMS\Table');
    ...
    public function cancel($key = null)
    {
        ...
        if ($recordId)
        {
            if ($checkin)
            {
                if ($model->checkin($recordId) === false)
                {
                    ...
                    $this->setRedirect(
                        Route::_($this -> getAddonRedirect(). $this->getRedirectToItemAppend($recordId, $key)
                            , false
                        )
                    );
        ...
        $this->setRedirect(
            Route::_($this -> getAddonRedirect($this->view_list). $this->getRedirectToListAppend()
                , false
            )
        );
    ...
    public function edit($key = null, $urlVar = null)
    {
        ...
        if ($checkin && !$model->checkout($recordId))
        {
            ...
            $this->setRedirect(
                Route::_($this -> getAddonRedirect(). $this->getRedirectToItemAppend($recordId, $urlVar)
                    , false
                )
            );
    ...
    public function loadhistory()
    {
        ...
        if (!$model->loadhistory($historyId, $table))
        {
            ...
            $this->setRedirect(
                Route::_($this -> getAddonRedirect($this->view_list). $this->getRedirectToItemAppend()
                    , false
                )
            );
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if (!$model->save($validData))
        {
            ...
            $view           = $this -> input -> getCmd('view');
            $this->setRedirect(
                Route::_('index.php?option=' . $this->option . '&view='.$view
        ...
        switch ($task)
        {
            ...
            case 'save2new':
                ...
                // Redirect back to the edit screen.
                $this->setRedirect(
                    Route::_($this -> getAddonRedirect(). $this->getRedirectToItemAppend(null, $urlVar)
                        , false
                    )
                );
                break;
