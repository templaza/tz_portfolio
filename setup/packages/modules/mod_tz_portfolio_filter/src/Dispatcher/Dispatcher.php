...
namespace TemPlaza\Component\TZ_Portfolio\Module\Filter\Site\Dispatcher;
...
class Dispatcher extends AbstractModuleDispatcher implements HelperFactoryAwareInterface
{
    ...
    protected function getLayoutData()
    {
        ...
        $helper = $this -> getHelperFactory()-> getHelper('FilterHelper', $data);
...
//        $data['list'] = $helper -> getList($data['params'], $this->getApplication());
        ...
        $data['advfilter']       = $helper -> getAdvFilterFields($params);
        $data['categoryOptions'] = $helper -> getCategoriesOptions($params, $this->getApplication());

        if($params -> get('enable_uikit', 1)){
            $wa -> usePreset('com_tz_portfolio.uikit');
        }

//        $data['button_text']    = htmlspecialchars($params->get('button_text',
