...
//				$query->join('LEFT', '#__content_rating AS v ON a.id = v.content_id');
                ...
                $subquery .= 'WHERE parent.extension = ' . $db->quote('com_tz_portfolio');
                ...
                if (empty($data)) {
                    throw new \Exception(Text::_('COM_TZ_PORTFOLIO_ERROR_ARTICLE_NOT_FOUND'), 404);
                }
...
//                if($category   = CategoriesHelper::getCategoriesById($data -> catid)) {
...
//                        if($inheritCategory = CategoriesHelper::getCategoriesById($inheritCatid)){
                            $inheritParams  = new Registry();
        ...
        $author_info -> url                                 = $author_registry -> get('tz_portfolio_user_url');
        $author_info -> email                               = $item -> author_email;
        $author_info -> gender                              = $author_registry -> get('tz_portfolio_user_gender');
        $author_info -> avatar                              = $author_registry -> get('tz_portfolio_user_avatar');
        $author_info -> twitter                             = $author_registry -> get('tz_portfolio_user_twitter');
        $author_info -> facebook                            = $author_registry -> get('tz_portfolio_user_facebook');
        $author_info -> instagram                           = $author_registry -> get('tz_portfolio_user_instagram');
        $author_info -> googleplus                          = $author_registry -> get('tz_portfolio_user_googleplus');
        $author_info -> description                         = $author_registry -> get('tz_portfolio_user_description');
