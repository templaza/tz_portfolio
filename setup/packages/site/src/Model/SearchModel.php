...
class SearchModel extends ListModel
{
    ...
    protected function getListQuery(){
        ...
        if($searchWord || count($fields) || (!$searchWord && !count($fields) && $catid)) {
            ...
            $query->select('parent.alias as parent_alias');
...
//            HTMLHelper::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_tz_portfolio/helpers/html');
    ...
    public function getAdvFilterFields(){
        ...
        if(count($groupIds)){
            if($advfilter = ExtraFieldsFrontHelper::getAdvFilterFields(null, array(
