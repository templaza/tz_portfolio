...
class PortfolioModel extends ListModel
{
    ...
    protected function populateState($ordering = null, $direction = null){
        ...
        $global_params    = ComponentHelper::getParams('com_tz_portfolio');
        ...
        $this -> setState('filter.igCatId', $app -> input -> get('igCatId'));
        $this -> setState('filter.igTagId', $app -> input -> get('igTagId'));

        $articleOrderDate	= $params->get('order_date', 'created');

        $orderby    = '';
        $secondary  = QueryHelper::orderbySecondary($params -> get('orderby_sec', 'rdate'), $articleOrderDate);
    ...
    protected function getListQuery(){
        ...
        $query -> select(
            $this->getState(
                ...
                ',CASE WHEN CHAR_LENGTH(cc.alias) AND m.main=1 THEN CONCAT_WS(":", cc.id, cc.alias) ELSE cc.id END as catslug'.
...
//                ',parent.title as parent_title, parent.id as parent_id, parent.path as parent_route, parent.alias as parent_alias'
        ...
        $tagAlias   = $this -> getState('filter.tagAlias');
        ...
        if($tagId || $tagAlias) {
            ...
            if($tagAlias) {
                ...
                //            $query -> join('INNER',$db -> quoteName('#__tz_portfolio_plus_plus_tags').' AS t2 ON t2.id=x.tagsid');
        ...
        if ($fields = $this->getState('filter.fields')) {
            if (count($fields)) {
                ...
                if ($extraFields = ExtraFieldsFrontHelper::getExtraFieldObjectById($fieldIds)) {
                    $where = array();
...
//        $query -> group('c.id');
    ...
    public function getItems(){
        ...
        if($items = parent::getItems()){
            ...
            if(count($items)>0){
                ...
                $globalParams = ComponentHelper::getParams('com_tz_portfolio', true);
                ...
                foreach($items as $i => &$item){
                    ...
                    if(COM_TZ_PORTFOLIO_JVERSION_4_COMPARE && !empty($item -> params)) {
                    ...
                    // Create Article Link
                    $item ->link        = Route::_(RouteHelper::getArticleRoute($item -> slug, $item -> catid, $item->language).$tmpl);
                    $item -> fullLink   = Route::_(RouteHelper::getArticleRoute($item -> slug, $item -> catid, $item->language), true, $ssl);
    ...
    public function ajax($data = null){
        ...
        $igCatId_filter = $data['igCatId'];
        $igTagId_filter = $data['igTagId'];

        if(!empty($igCatId_filter)){
            $igCatId_filter   = ArrayHelper::toInteger($igCatId_filter);
        }
        if(!empty($igTagId_filter)){
            $igTagId_filter   = ArrayHelper::toInteger($igTagId_filter);
        }
        ...
        $this -> setState('filter.igCatId',$igCatId_filter);
        $this -> setState('filter.igTagId',$igTagId_filter);
    ...
    public function getCategoriesByArticle(){
        if(isset($this -> cache[$this->getStoreId()]) && ($articles = $this -> cache[$this->getStoreId()])){
            ...
            if($igCatId_filter = $this -> getState('filter.igCatId')){
                $options['filter.igCatId']  = $igCatId_filter;
            }

            if(!$params -> get('filter_second_category', 1)){
                return CategoriesHelper::getMainCategoriesByArticleId($contentId);
            }

            return CategoriesHelper::getCategoriesByArticleId($contentId, $options);
        }
        return false;
    }
    ...
    public function getAllCategories(){
        ...
        if(count($catid) && $categories = CategoriesHelper::getCategoriesById($catid, $options)){
            return $categories;
        }
        return CategoriesHelper::getAllCategories($options);
    }
    ...
    public function getTagsByArticle($filterAlias = null){
        if(isset($this -> cache[$this->getStoreId()]) && ($articles = $this -> cache[$this->getStoreId()])){
            ...
            if($igTagId_filter = $this -> getState('filter.igTagId')){
                $options['filter.igTagId']  = $igTagId_filter;
            }

            $tags   = TagHelper::getTagsFilterByArticleId($contentId, $filterAlias, $options);
    ...
    public function getAllTags(){
        $params = $this -> getState('params');
        return TagHelper::getTagsByCategoryId($params -> get('catid'));
    }
    ...
    public function getAvailableLetter(){
        ...
        if($params -> get('use_filter_first_letter',0)){
            if($letters = $params -> get('tz_letters','a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z')){
                ...
                $lettersArt = ArticleHelper::getLetters($filters);
                ...
                foreach($letters as $i => &$letter){
                    ...
                    $arr[$i]    = false;
                    if(!empty($lettersArt) && in_array($letterKey, $lettersArt)){
                        $arr[$i]    = true;
                    }
                }
