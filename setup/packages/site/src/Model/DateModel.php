...
class DateModel extends ListModel
{
    ...
    public function populateState($ordering = null, $direction = null){
        ...
        $orderby    = '';
        $secondary  = QueryHelper::orderbySecondary($params -> get('orderby_sec', 'rdate'), 'created', 'a');
        $primary    = QueryHelper::orderbyPrimary($params -> get('orderby_pri'), 'c');
...
//        $this->setState('list.limit', $limit);
    ...
    function getListQuery(){
        ...
        if ($this->getState('filter.language')) {
            ...
            $query->where('(contact.language in ('.$db->quote(Factory::getLanguage()->getTag()).','.$db->quote('*').') OR contact.language IS NULL)');
        }
    ...
    public function getItems(){
        if($items	= parent::getItems()){
            ...
            $globalParams = ComponentHelper::getParams('com_tz_portfolio_plus', true);
