...
class TagHelper{
    ...
    public static function getTagsFilterByArticleId($contentId, $filterAlias = null, $options = array()){

        $storeId    = __METHOD__;
        $storeId   .= '::'.serialize($contentId);
        $storeId   .= '::'.serialize($filterAlias);
        ...
        if(isset($options['filter.igTagId']) && !empty($options['filter.igTagId'])){
            if(is_array($options['filter.igTagId'])){
                $query -> where('t.id NOT IN('.implode(',', $options['filter.igTagId']).')');
            }else{
                $query -> where('t.id <> '.((int)$options['filter.igTagId']));
            }
        }
        ...
        if($contentId) {
            ...
            if(!isset(self::$cache[$storeId])){
                ...
                if($data = $db -> loadObjectList()){
                    ...
                    foreach($data as &$tag){
                        // Create Tag Link
                        $tag -> link    = Route::_(RouteHelper::getTagRoute($tag -> slug, 0, $menuActive));
