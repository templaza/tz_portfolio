...
class CategoriesHelper{
        ...
        if($articleId) {
            ...
            if(!isset(self::$cache[$storeId])){
                ...
                if(count($config)){
                    ...
                    if(isset($config['filter.igCatId']) && !empty($config['filter.igCatId'])){
                        if(is_array($config['filter.igCatId'])){
                            $query -> where('c.id NOT IN('.implode(',', $config['filter.igCatId']).')');
                        }else{
                            $query -> where('c.id <> '.((int)$config['filter.igCatId']));
                        }
                    }
                }
            ...
            if(!isset(self::$cache[$storeId])){
                ...
                if(is_array($id)){
                    ...
                    if($categories = $db -> loadObject()) {
                        $categories -> order    = 0;
                        $categories->link = Route::_(RouteHelper::getCategoryRoute($categories->id));
                    }
                }
                if($categories){
                    self::$cache[$storeId]  = $categories;
    ...
    public static function getAllCategories($options = array('second_by_article' => false, 'orderby' => null)){
        ...
        $config = array('second_by_article' => false, 'orderby' => null);
        $config = array_merge($config, $options);

        $storeId    = __METHOD__;
        $storeId   .= ':'.serialize($config);
        $storeId    = md5($storeId);
        ...
        if(count($config)){
            if(isset($config['second_by_article']) && $config['second_by_article']){
                $query -> where('(m.main = 0 OR m.main = 1)');
            }else{
                $query -> where('m.main = 1');
            }
            if(isset($config['filter.parent']) && $config['filter.parent']){
                $query->join('LEFT', $db->quoteName('#__tz_portfolio_plus_categories') . ' AS p ON p.id = ' . (int) $config['filter.parent'])
        ...
        if($categories = $db -> loadObjectList()){
            ...
            return self::$cache[$storeId]  = $categories;
        }
        return false;
    }
}