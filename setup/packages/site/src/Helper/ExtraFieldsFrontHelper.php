...
class ExtraFieldsFrontHelper{
    ...
    public static function getExtraField($field, $article = null, $resetArticleCache = false, $options = array())
    {
        ...
        if (!isset(self::$cache['fields'][$storeId]))
        {
            ...
            $fieldClass = AddonHelper::getInstance('extrafields', $field -> type);

            if($fieldClass instanceof PluginInterface){
                if (!self::checkExtraField($field-> type))
                {
                    $namespace  = 'TemPlaza\\Component\\TZ_Portfolio\\Administrator\\Library\\Common';
                    $className  = $namespace.'\\ExtraFieldCommon';
                }
                else
                {
                    $namespace  = 'TemPlaza\Component\TZ_Portfolio\AddOn\\Extrafields\\'. ucfirst($field->type);
                    $className  = $namespace.'\Extension\\'. ucfirst($field->type);
                }

                self::registerExtraFieldNamespace($field -> type);
                ...
                {
                    $fieldClass = new $className($_fieldObj, null, $options);
                }
            }else{
                $_fieldObj = clone $field;

                if(!empty($fieldClass)){
                    $fieldClass->setField($_fieldObj);
                }
            }
    ...
    public static function registerExtraFieldNamespace($name){
        $fname      = ucfirst($name);
        $namespace  = 'TemPlaza\Component\TZ_Portfolio\AddOn\Extrafields\\'.$fname;
        $className  = $namespace.'\\Extension\\'.$fname;

        if(!class_exists($className)) {
            $path   = COM_TZ_PORTFOLIO_ADDON_PATH . '/extrafields/' . $name . '/src';
    ...
    public static function getExtraFieldsByArticle($article, $params = null, $options = array()){
        ...
        {
            $article = TZ_PortfolioContentHelper::getArticleById($article);
        }
        $storeId    = __METHOD__;
