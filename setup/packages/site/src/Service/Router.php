...
class Router extends RouterView
{
    ...
    public function getAddonRouter($addon, $group = 'content'){
        if (!isset($this->addonRouters[$addon]))
        {
            ...
            $class      = 'PlgTZ_Portfolio'.ucfirst($group).$addonname . 'Router';


            if (!class_exists($class))
            {
                // Use the component routing handler if it exists
                $path   = JPATH_SITE.'/components/com_tz_portfolio/add-ons/'.$group.'/'.$addon.'/router.php';
    ...
    protected function sefBuild(&$query){
        ...
        // Check again
        if ($menuItemGiven && isset($menuItem) && $menuItem->component != 'com_tz_portfolio')
        {
            $menuItemGiven = false;
            unset($query['Itemid']);
        }
    ...
    protected function _getUserSegment(&$query, &$segments){
        ...
        if(isset($query['uid'])){
            ...
            $alias  = '';
            if($params -> get('sef_use_user_alias',1) && !empty($user -> name)) {
...
//            }
        ...
        if($item) {
            ...
            if ($item->query['view'] == 'portfolio') {
                $category = Categories::getInstance('TZ_Portfolio')->get($id);

                if (!$category) {
                    throw new \Exception(Text::_('COM_TZ_PORTFOLIO_ERROR_PARENT_CATEGORY_NOT_FOUND'), 404);
                    return $vars;
                }
