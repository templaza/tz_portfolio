...
namespace TemPlaza\Component\TZ_Portfolio\AddOn\Mediatype\Image\Model;
...
use TemPlaza\Component\TZ_Portfolio\Administrator\Library\ImageWaterMark;
use TemPlaza\Component\TZ_Portfolio\Site\Helper\CategoriesHelper;
use TemPlaza\Component\TZ_Portfolio\Site\Helper\TZ_PortfolioHelper;
    ...
    public function save($data){
            ...
            if(is_array($image_size) && count($image_size)){
                foreach($image_size as $_size){
                    ...
                    if((isset($image_data['url_remove']) && $image_data['url_remove'])
                        ...
                        $image_url  = TZ_PortfolioHelper::getImageURLBySize($media -> url,
                        ...
                        $murl       = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_BASE.'/'.$alias;
                    ...
                    if((isset($image_data['url_detail_remove']) && $image_data['url_detail_remove'])
                        ...
                        $image_url  = TZ_PortfolioHelper::getImageURLBySize($media -> url_detail,
                            $size ->image_name_prefix);
        ...
        if(isset($image_data['url_remove']) && $image_data['url_remove']){
            ...
            if($media && isset($media -> url) && !empty($media -> url)){
                ...
                $image_url  = TZ_PortfolioHelper::getImageURLBySize($media -> url, 'o');
        ...
        if(isset($image_data['url_detail_remove']) && $image_data['url_detail_remove']){
            ...
            if($media && isset($media -> url_detail) && !empty($media -> url_detail)){
                ...
                $image_url  = TZ_PortfolioHelper::getImageURLBySize($media -> url_detail, 'o');
...
//        jimport('joomla.filesystem.file');
        ...
        if(!is_dir(COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT)){
            Folder::create(COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT);
        }
        ...
        if(count($images) && !empty($images['tmp_name'])) {
            ...
            $path   = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT.DIRECTORY_SEPARATOR;
        ...
        if(count($images_hover) && !empty($images_hover['tmp_name'])) {
            ...
            $path_hover     = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT.DIRECTORY_SEPARATOR;
            ...
            if(is_array($image_size) && count($image_size)){
                foreach($image_size as $_size){
                    ...
                    if($path) {
                        ...
                        $newPath = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT . DIRECTORY_SEPARATOR
...
//                        // Add watermark for each image size
//                        $this -> watermark(TZ_PortfolioHelper::getImageURLBySize(JPATH_ROOT
...
//                    }
                    ...
                    if($path_hover) {
                        ...
                        $newHPath           = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT . DIRECTORY_SEPARATOR
...
//                    }
        ...
        if($path && !empty($path)){
            ...
            $image_data['url']   = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_BASE.'/'
                .$data -> alias . '-' . $data -> id. '.' . File::getExt($path);
        }
//        elseif(isset($media -> url) && $media -> url){
//            $this -> watermark(JPATH_ROOT.DIRECTORY_SEPARATOR
//                .TZ_PortfolioHelper::getImageURLBySize($media -> url, 'o'), 'o');
//        }
        ...
        if($path_hover && !empty($path_hover)){
            ...
            $image_data['url_detail']   = COM_TZ_PORTFOLIO_MEDIA_ARTICLE_BASE.'/'
                .$data -> alias . '-' . $data -> id. '-h.' . File::getExt($path_hover);
        }
//        elseif(isset($media -> url_detail) && $media -> url_detail
//            && $params -> get('mt_image_watermark_img_detail', 0)){
//            $this -> watermark(JPATH_ROOT.DIRECTORY_SEPARATOR
//                .TZ_PortfolioHelper::getImageURLBySize($media -> url_detail, 'o'), 'o');
//        }
...
//        }
    ...
    protected function watermark($file, $imgType = '', $fontSize = 0, $cordinates = ''){
        ...
        if(isset($layer) && $layer){
            ...
            $mainLayer -> save(COM_TZ_PORTFOLIO_MEDIA_ARTICLE_ROOT,basename($file), true, null, 100);
        }
    }
}