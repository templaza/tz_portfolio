...
TZ_PortfolioAddOnContentVote = window.TZ_PortfolioAddOnContentVote || {
    "basePath": "",
    "addonId": 0,
    "articleId": 0,
    "referView": "",
...
(function($, document, window, Joomla){
	...
	$.tzPortfolioAddOnVote	= function(el, options){
		var addonVote = $.tzPortfolioAddOnVote,
            $el	= $(el), reg = /\/$/,
            settings = $.extend(true,$.extend(true,{},$.tzPortfolioAddOnVote.defaults),options),
            clicked = false, loadingHtml = settings.loadingTemplate,
            notifyOptions = settings.notification,
            basePath = settings.basePath;

        var addOnContentVote = Joomla.getOptions('com_tz_portfolio.addon.content.vote');
        ...
        $el.processVote = function($item){
            ...
            if (settings.ajaxUrl) {
                ...
                url = "index.php?option=com_tz_portfolio&view=addon&addon_task=vote.vote";
            ...
            $.ajax({
                ...
                UIkit.notification({
                    ...
                    status: ((result.success == true) ? "success" : "danger"),
                    pos: 'top-right',
                    timeout: notifyOptions.effect
                });
        ...
        // if(typeof TZ_Portfolio.infiniteScroll !== undefined) {
        //     if(typeof TZ_Portfolio.infiniteScroll.addAjaxComplete !== "undefined") {
        //         TZ_Portfolio.infiniteScroll.addAjaxComplete(function (newElements, masonryContainer) {
        //             var $container = newElements.find(settings.mainSelector);
        //
        //             $container.tzPortfolioAddOnVote(settings);
        ...
        $el.vars    = settings;
        $.data(el,"tzPortfolioAddOnVote", $el);

        return this;
	};
    $.tzPortfolioAddOnVote.originalJS   = false;

	$.tzPortfolioAddOnVote.defaults	= {
	    basePath: "",
		ajaxUrl: "",
        ajaxType: "POST",
        ajaxDataType: "json",
	...
	$.fn.tzPortfolioAddOnVote = function (options) {
        if (options === undefined) options = {};
        if (typeof options === "object") {
            // Call function
            return this.each(function(index, value) {
                // Call function
                if ($(this).data("tzPortfolioAddOnVote") === undefined) {
                    new $.tzPortfolioAddOnVote(this, options);
                }else{
                    $(this).data('tzPortfolioAddOnVote');
                }
            });
        }
    };
    ...
    window.JVXVote = function(obj,artId,ratePoint,rateTotal, rateCount,showCounter,styleType) {
        $.tzPortfolioAddOnVote.originalJS   = true;

        var $item   = $(obj),
            $vote = $item.data("tzPortfolioAddOnVote");
        if($vote !== undefined){
            $vote.processVote($item);
        }
        $item.tzPortfolioAddOnVote({
            mainSelector: ".content_rating",
            itemSelector: ".rating > a",
...
})(jQuery, document, window, Joomla/*, TZ_Portfolio, TZ_PortfolioAddOnContentVote*/);