...
if ( $params->get('access') == 1 && !$user->get('id') ) {
	...
	if ($user_rating >= 1 && $user_rating <= 5) {
		...
		$query -> select('*');
		$query -> from('#__tz_portfolio_plus_plus_content_rating');
		...
		try{
            if ( !$votesdb ) {
                $query -> clear();
                $query -> insert('#__tz_portfolio_plus_plus_content_rating');
                ...
                if ($currip != ($votesdb->lastip)) {
                    $query -> clear();
                    $query -> update('#__tz_portfolio_plus_plus_content_rating');
